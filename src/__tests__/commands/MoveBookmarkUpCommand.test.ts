...
describe('MoveBookmarkUpCommand', () => {
  ...
  beforeEach(() => {
    ...
    command = new MoveBookmarkUpCommand(bookmarkManager, storageService, treeDataProvider, collectionManager);
  ...
  describe('execute', () => {
    it('should move bookmark up successfully', async () => {
      ...
      expect(bookmarkManager.moveBookmarkUp).toHaveBeenCalledWith('file:///test/file.ts', 10);
    ...
    it('should show error message when bookmark cannot be moved up', async () => {
      ...
      expect(showInformationMessageSpy).toHaveBeenCalledWith('Bookmark is already at the top or cannot be moved');
